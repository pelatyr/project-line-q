    ...
    // if ( isset($_POST["lineId"]) && !empty($_POST["lineId"]) ) {
        ...
        $sql = " 
            ...
            FROM (
                SELECT
                    ...
                    customer.*,
                    CASE 
                        WHEN MOD(ROW_NUMBER() OVER(ORDER BY q.queue ASC), 1) = 0
                        THEN
                            ROW_NUMBER() OVER(ORDER BY q.queue ASC)
                        ELSE
                            NULL
                    END AS mod_row
                FROM
                    q
                INNER JOIN customer ON q.customer_ID = customer.customer_line
                WHERE
                    q.queue >= {$_POST["queue"]}
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = mysqli_fetch_array($result)) {
                $payload["body"]["to"] = $row["customer_line"];
                $payload["body"]["messages"][0]["type"] = "text"; 
                $payload["body"]["messages"][0]["text"] = "เหลืออีก {$row["mod_row"]} คิว ถึงคิวของท่าน";
                pushMessage($payload);
                $i++;
            }
        }
